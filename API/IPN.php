...
class IPN
{
    ...
    protected function handleIpn($data)
    {
        ...
        do_action('wppayform_ipn_mollie_action_'.$status, $submission, $vendorTransaction);

        if ($refundAmount = Arr::get($vendorTransaction, 'amountRefunded.value')) {
            $refundAmount = intval($refundAmount * 100); // in cents
            do_action('wppayform_ipn_mollie_action_refunded', $refundAmount, $submission, $vendorTransaction);
        }
    }

    public function makeApiCall($path, $args, $formId, $method = 'GET')
    {
        $apiKey = (new MollieForPaymattic\Settings\MollieSettings())->getApiKey($formId);
